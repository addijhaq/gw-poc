...
import com.bcg.humana.orderservice.model.OrderResponse;
...
public class OrderController {
  ...
  public ResponseEntity<Object> getOrderByCustomerId(@PathVariable long customerId) {
    try{
      ...
      return orders.size() == 0 ? new ResponseEntity<>("No orders found", HttpStatus.NOT_FOUND) : new ResponseEntity<>(orders, HttpStatus.OK);
    }catch (Exception e) {
    ...
    try{
      return new ResponseEntity<>(Collections.singletonList(
          orderRepository.findById(orderId)
              .orElseThrow(() -> new Exception("Cannot find order with id " + orderId))), HttpStatus.OK);
    }catch (Exception e) {
      return new ResponseEntity<>("Cannot process request", HttpStatus.NOT_FOUND);
    }
  }
